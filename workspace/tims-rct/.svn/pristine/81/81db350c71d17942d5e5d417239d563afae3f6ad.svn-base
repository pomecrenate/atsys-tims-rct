...
<script>
const tbColNms = [
	  { type: "checkbox", name: "annCd"},
	  { type: "text", name: "annTitle" },
	  { type: "text", name: "annCnts" },
	  { type: "button", name: "editBtn", id: "editBtn", btnText: "수정", dataAction: "annPopUpd"},
];
...
function search() {
	const param = TimsUtil.formToMap('search');
	const ajax = TimsUtil.getObject("${svnmSearch}", param, false);
...
$(document).on("click", "button[data-action]", function () {
	...
	if(action === "annPopIns") {
		var url = '${svnmAnnPop}'; 
		RctUtil.urlDialog('알림 문구 등록/수정', url, 800, 800);  
		...
		const cnts = $tr.find("td").eq(2).text().trim();
		
		$("#annTitle").val(title);
		$("#annCnts").val(cnts);
		$("#annCd").val($tr.data('id'));
	}
})
</script>