...
const tbColNms = [
	  ...
	  { type: "text", name: "updDate", dataField : "updDate" },
 ...
 $(document).on("click", "button[data-action]", function () {
 	...
 	switch (action) {
 		...
 		case "careerRecogPop" :
 			...
 			RctUtil.urlDialog('경력산정(직원전용)', url, 800, 400);  
 			break;
...
$(document).on("click", "button[data-action='search']", function() { 
	const params = {
						'nctYear' : $("#qryNtcYear").val(), 
						'ntcSmt' : $("#qryNtcSmt").val(),
						'ntcCd' : $("#qryNtcCd").val(),
						'staffType' : $("#qryStaffType").val(),
						'collegeCd' : $("#qryCollegeCd").val(),
						'deptCd' : $("#qryDeptCd").val(),
						'appStatus' : $("#qryAppStatus").val() 
					} 
