...
if(!window["GearFile"])
{
	...
	window["GearFile"] = function(){
		return {
			...
			fileMap		: {
				...
				getDeletedFiles:function()
				{
					...
					{
						var obj ={
							fileKey : this.delArray[i].upFile.fileKey,
							fileGroupKey : this.delArray[i].upFile.fileGroupKey,
							fileStatus : this.delArray[i].upFile.fileStatus
						};
						arr.push(obj);
					}
					return arr;
		...
		//		var url = "jQuery{servBasePath}/file/files?_method=DELETE";
			...
			removeFile :function(guid)
			{
				...
				{
					var url = this.deleteUrl+data.upFile.fileKey
			...
			setPreview:function(data)
			{
				...
				if(data.upFile)
				{
					if(data.upFile.fileKey && this.downloadUrl){
						var src = this.downloadUrl+data.upFile.fileKey;
			...
			setInfomations:function(){
				var info= this.divFileSlt.find("[data-type=gear-info]");
			...
			getHeadTemplate : function()
			{
				...
				if(bEditMode){
			        ...
			        sHtml+=		'    		</div>';
			        sHtml+=		'    		<div class="col-lg-5 fileupload-progress">';
			        ...
			        sHtml+=		'        		</div>';
			        sHtml+=		'        		<div class="progress-extended" data-type="gear-info" >0 KB / 0 KB | 0 file(s) /0 file(s) | 0 % </div>';
			        sHtml+=		'		</div>';
			        sHtml+=		'	</div>';
				}
			...
			downloadFile: function(guid){
				...
				ifm.attr("src",this.downloadUrl+data.upFile.fileKey);
			...
			uploadFile :function(guid)
			{
				...
				.success(function (result, textStatus, jqXHR) {
					if(gearFile.success)
					{
						if(window[gearFile.success])
						{
							window[gearFile.success](gearFile,result);
							return;
						}
					}
					console.log("success");
	            ...
	            .complete(function (result, textStatus, jqXHR) {
					...
					if(gearFile.autoDelete)
						gearFile.removeFile(data.guid);
					gearFile.setInfomations();
...
//						data.jqXHR.abort();
				...
				data.guid=TimsUtil.guid();
						...
						if(!hasStart)
						{
							...
							jQuery(this).click(function(e){
								...
								if(jQuery(this).attr("uploading"))
									return;
								gearFile.uploadFile(jQuery(this).attr("guid"));
							});
						}
					});
					this.divFileSlt.find("button[file-action=cancel][guid]").each(function(idx){
						...
						if(!hasCancel)
						{
							...
							jQuery(this).click(function(e){
								e.preventDefault();
								gearFile.cancelUpload(jQuery(this).attr("guid"));
					...
					this.divFileSlt.find("button[file-action=delete][guid]").each(function(idx){
						...
						if(!hasDelete)
						{
							...
							jQuery(this).click(function(e){
								e.preventDefault();
								gearFile.deleteUpload(jQuery(this).attr("guid"));
			...
			initUpload:function(){
				...
				paramName: 'files',
			    formData : [{ name: 'fileGroupKey', value: this.fileGroupKey },
			    ...
			    fileGroupKey : this.fileGroupKey,
...
//					url : url + "/tempUpload",
				...
				change: function (e, data) {
					var accept = gearFile.accpetFileTypes&&gearFile.accpetFileTypes!="*"?gearFile.accpetFileTypes.split("|"):null;
					var except  = gearFile.exceptFileTypes?gearFile.exceptFileTypes.split("|"):null;
						
					this.error = false;
					var cnt = gearFile.getFileCount()+ data.files.length;
					if(gearFile.fileMaxCnt>0&&cnt>gearFile.fileMaxCnt)
					{
						this.error=true;
						alert("최대 "+gearFile.fileMaxCnt + "개의 파일만 업로드 할 수 있습니다");
					}
					var totalSize = gearFile.getTotalFileSize();
					for(var i=0;i<data.files.length;i++)
					{
						var ext = data.files[i].name.split('.').pop().toLowerCase();
						var fSize = data.files[i].size;
						 if(gearFile.fileMaxSingleSize>0&& fSize>gearFile.fileMaxSingleSize)
						 {
							 this.error=true;
							 var fMsg =TimsUtil.getReadableFileSizeString(gearFile.fileMaxSingleSize);
							 alert("파일 하나당 최대 "+fMsg+ "까지 업로드 가능합니다.");
							 break;
						 }
						totalSize+=fSize;
						if(gearFile.fileMaxSize>0&& totalSize>gearFile.fileMaxSize)
						{
							 this.error=true;
							 var fMsg =TimsUtil.getReadableFileSizeString(gearFile.fileMaxSingleSize);
				...
				add : function(e, data) {
					if(this.error)
						return;
					gearFile.addTempFile(data);
					if(gearFile.autoUpload)
					{
						gearFile.uploadFile(data.guid);
					}
				...
				progress: function(e, data) {
					...
					var prgrsSlt = gearFile.divFileSlt.find('[name=fileProgress][guid='+guid+']');
			...
			loadServerFile : function(file){
				...
				this.divFileSlt.find("button[file-action=download][guid]").each(function(idx){
					...
					if(!hasDownload)
					{
						...
						jQuery(this).click(function(e){
							e.preventDefault();
							gearFile.downloadFile(jQuery(this).attr("guid"));
			...
			init : function(option){
				var gearFile = this;
				this.fileMap.gearFile=gearFile;
				//GearFile properties 초기화
				if(option)
...
//					return false;
	...
	jQuery.fn.gearFile=function(){
		if(!this.length)
			return null;
		if(!window["gearFiles"])
			return null;
		var id= this.attr("id");
		if(id&&window["gearFiles"][id])
			return window["gearFiles"][id];
		var option = this.data();
		if(!option.divFile)
		{
			if(!jQuery(this).attr("id"))
			{
				jQuery(this).attr("id","gearFile"+idx);
			}
	...
	jQuery(document).ready(function(){
		jQuery("[data-type='gear-file']").each(
		function(idx)
		{
			jQuery(this).gearFile();
