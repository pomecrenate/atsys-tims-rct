        ...
        (Multiselect.prototype = {
            defaults: {
                buttonText: function (selectedOptions, select) {
                    ...
                    if (this.allSelectedText && selectedOptions.length === $("option", $(select)).length && 1 !== $("option", $(select)).length && this.multiple)
                        return this.selectAllNumber ? this.allSelectedText + " (" + selectedOptions.length + ")" : this.allSelectedText;
                ...
                this.$select.children().each(
                    $.proxy(function (index, element) {
                        ...
                        $element.prop("value") !== this.options.selectAllValue &&
                            ("optgroup" === tag ? this.createOptgroup(element) : "option" === tag && ("divider" === $element.data("role") ? this.createDivider() : this.createOptionValue(element, !1)));
                    }, this)
                ),
                ...
                if (this.options.enableFiltering || this.options.enableCaseInsensitiveFiltering) {
                    ...
                    this.$select.find("option").length >= enableFilterLength &&
                        ...
                        this.options.includeFilterClearBtn
                            ? (this.isFirefox() && 0 === this.$filter.find(".multiselect-clear-filter").length && this.$filter.append("<i class='fas fa-times text-muted multiselect-clear-filter multiselect-moz-clear-filter'></i>"),
                            ...
                            : (this.$filter.find(".multiselect-search").attr("type", "text"), this.$filter.find(".multiselect-clear-filter").remove()),
